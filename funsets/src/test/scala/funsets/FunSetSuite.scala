...
class FunSetSuite extends FunSuite {
  ...
  test("intersect test") {
    new TestSets {
      val ss = intersect(union(s1, s2), union(s3, s2))
  ...
  test("diff test") {
    new TestSets {
      val diffSet = diff(union(s1, s2), union(s3, s2))
  ...
  test("filter test") {
    new TestSets {
      val bigUnion = union(s1, union(s2, union(s3, singletonSet(4))))
      val filtered = filter(bigUnion, _ > 2)
  ...
  test("exist test") {
    new TestSets {
      val bigUnion = union(s1, union(s2, union(s3, singletonSet(4))))
      assert(!exists(bigUnion, _ == 5))
      assert(exists(bigUnion, _ == 3))
    }
  }
